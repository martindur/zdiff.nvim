...
local function get_diff_stats_async(base_ref, done)
  ...
  if base_ref then
    diff_target = base_ref .. get_diff_mode_separator() .. "HEAD"
  end

  local flags = get_diff_flags(false)
  local numstat_args = { "git", "diff", "--numstat" }
  vim.list_extend(numstat_args, flags)
  table.insert(numstat_args, diff_target)

  local stats = {}
  run_command_async(numstat_args, function(_, result)
    ...
    local status_args = { "git", "diff", "--name-status" }
    vim.list_extend(status_args, flags)
    table.insert(status_args, diff_target)
    run_command_async(status_args, function(_, status_result)
      ...
      if base_ref or files_cfg.include_untracked == false then
        done(stats)
        return
      end
